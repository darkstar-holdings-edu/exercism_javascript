...
describe('Hamming', () => {
  ...
  test('disallow first strand longer', () => {
    expect(() => compute('AATG', 'AAA')).toThrow(new Error('strands must be of equal length'));
  });

  test('disallow second strand longer', () => {
    expect(() => compute('ATA', 'AGTG')).toThrow(new Error('strands must be of equal length'));
  });

  test('disallow empty first strand', () => {
    expect(() => compute('', 'G')).toThrow(new Error('strands must be of equal length'));
  });
